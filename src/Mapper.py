...
from src.common import (get_camera_from_tensor, get_samples, get_tensor_from_camera, random_select, depth_fusion)
from src.utils.datasets import get_dataset
from src.utils.Visualizer import Visualizer
# from src.deeppruner_simplify.deeppredict import deeppredict
...
class Mapper(object):
    ...
    def __init__(self, cfg, args, slam, coarse_mapper=False
        ...
        self.deeppredictor  = slam.deeppredictor 
        ...
        # TODO:
        self.ignore_edge_W = cfg['mapping']['ignore_edge_W']
        self.ignore_edge_H = cfg['mapping']['ignore_edge_H']
        self.crop_edge = cfg['cam']['crop_edge']
        self.png_depth_scale = cfg['cam']['png_depth_scale']
        self.decay_scale = cfg['mapping']['decay_scale']
        self.decay_threshold = cfg['mapping']['decay_threshold']
        self.decay_second_threshold = cfg['mapping']['decay_second_threshold']
        self.depth_fusion_scale = cfg['Depthmodel']['depth_fusion_scale']
        ...
        # self.depth_model_path = "src/deeppruner_simplify/model/kiti_testfinetune_119.tar"
        # self.deeppredictor = deeppredict(self.depth_model_path)
    ...
    def keyframe_selection_overlap(self, gt_color, gt_depth, c2w, keyframe_dict, k, N_samples=16, pixels=100):
        ...
        random_index = torch.rand(1, device=device)
        rays_o, rays_d, gt_depth, gt_color = get_samples(random_index, 
                    Hedge, H-Hedge, Wedge, W-Wedge, pixels, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, self.device, flag = 'mapper')
    ...
    def optimize_map(self, num_joint_iters, lr_factor, idx, cur_gt_color, cur_gt_depth, gt_cur_c2w, keyframe_dict, keyframe_list, cur_c2w):
        ...
        for joint_iter in range(num_joint_iters):
            ...
            for frame in optimize_frame:
                ...
                #     0, H, 0, W, pixs_per_image, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, self.device, flag = 'mapper')
                # TODO:
                Wedge = self.ignore_edge_W
                Hedge = self.ignore_edge_H
                random_index = torch.rand(1, device=device)
                batch_rays_o, batch_rays_d, batch_gt_depth, batch_gt_color = get_samples(random_index,
                    Hedge, H-Hedge, Wedge, W-Wedge, pixs_per_image, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, self.device, flag = 'mapper')
            ...
            with torch.no_grad():
                ...
                # inside_mask = torch.logical_and(torch.logical_and(t >= batch_gt_depth, batch_gt_depth != 0.), batch_gt_depth < 10.)
                # inside_mask = torch.logical_and(t >= batch_gt_depth, batch_gt_depth < 15.)
    ...
    def run(self):
        ...
        idx, gt_color, gt_right_color, depth_data, gt_c2w = self.frame_reader[0]

        # TODO: DEEPPRUNER
        predict_depth = self.deeppredictor.run(gt_color, gt_right_color)
        
        gt_color, gt_right_color, depth_data = gt_color.cpu().numpy(), gt_right_color.cpu().numpy(), depth_data.cpu().numpy()
        gt_color = cv2.cvtColor(gt_color, cv2.COLOR_BGR2RGB)
        gt_color = gt_color / 255.
        gt_right_color = cv2.cvtColor(gt_right_color, cv2.COLOR_BGR2RGB)
        gt_right_color = gt_right_color / 255.

        depth_data = depth_data.astype(np.float32) / self.png_depth_scale

        H, W = depth_data.shape
        gt_color = cv2.resize(gt_color, (W, H))
        gt_right_color = cv2.resize(gt_right_color, (W, H))

        gt_color = torch.from_numpy(gt_color)
        gt_right_color = torch.from_numpy(gt_right_color)
        depth_data = torch.from_numpy(depth_data)*self.scale

        # predict_depth = (predict_depth.float()).div(self.png_depth_scale) * self.scale
        predict_depth = (predict_depth.float()) * self.scale
        
        edge = self.crop_edge
        if edge > 0:
            # crop image edge, there are invalid value on the edge of the color image
            gt_right_color = gt_right_color[edge:-edge, edge:-edge]
            gt_color = gt_color[edge:-edge, edge:-edge]
            depth_data = depth_data[edge:-edge, edge:-edge]
            predict_depth = predict_depth[edge:-edge, edge:-edge]
        
        gt_color = gt_color.to(self.device)
        gt_right_color = gt_right_color.to(self.device)
        ...
        # edge = self.crop_edge
        # predict_depth = predict_depth[edge:-edge, edge:-edge]
        # depth fusion function
        #TODO: fusion function(depth_data, predict_depth)
        gt_depth = depth_fusion(depth_data, predict_depth, self.depth_fusion_scale, self.device)
        ...
        while (1):
            ...
            _, gt_color, gt_right_color, depth_data, gt_c2w = self.frame_reader[idx]
            ...
            if self.low_gpu_mem:
                # print('Mapper cuda memory release')
                # print('#######################################################################')
