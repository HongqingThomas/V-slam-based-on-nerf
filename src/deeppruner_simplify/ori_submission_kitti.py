...
test_left_img, test_right_img = ls.datacollector(current_directory+args.datapath)
...
def main():
    ...
    print("imgR 1: ", np.array(imgR).shape)

    processed = preprocess.get_transform()
    print("processed 2: ",  processed)

    imgL = processed(imgL).numpy()
    imgR = processed(imgR).numpy()
    print("imgR 2: ", np.array(imgR).shape)

    imgL = np.reshape(imgL, [1, 3, imgL.shape[1], imgL.shape[2]])
    imgR = np.reshape(imgR, [1, 3, imgR.shape[1], imgR.shape[2]])

    print("imgR 3 ", np.array(imgR).shape)
    ...
    print("imgR 4 ", np.array(imgR).shape)

    print("imgR 5 ", np.array(imgR[:, :, top_pad:, :-left_pad]).shape)
    ...
    print("disparity: ", disparity.size())
    # print("top_pad: ", top_pad)
    # print("left_pad: ", left_pad)
    # disparity = disparity[top_pad:, :-left_pad]
    disparity=disparity[0].cpu().numpy().squeeze()
    disparity = (disparity * 1000.0).astype(np.uint16)
